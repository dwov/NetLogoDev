...
agents-own [
  ...
  perceived-grievance ; calculated perceived grievance of the agent
  estimated-arrest-probability ; calculated estimated arrest probability of the agent
]
...
to go
  ask agents [
    ...
    if jail-term = 0 [  ; Agents only deliberate if they are not in jail
      determine-behavior
    ]

    ; Execute actions (Means-Ends Reasoning)
    execute-action
...
to update-beliefs
  ; Calculate perceived grievance and estimated arrest probability
  set perceived-grievance perceived-hardship * (1 - government-legitimacy)
  set estimated-arrest-probability 1 - exp (- k * floor (count cops-on neighborhood / (1 + count (agents-on neighborhood) with [ active? ])))
end

to determine-behavior
  ; Deliberate what action to take based on beliefs
  ifelse (perceived-grievance - risk-aversion * estimated-arrest-probability > threshold) [
    set active? true  ; Become active (rebel)
  ] [
    set active? false  ; Remain quiet
  ]
end
...
to display-agent-2d  ; agent procedure
  ...
  ifelse active?
    ...
    [ ifelse jail-term > 0
        [ set color black + 3 ]
        [ set color scale-color green perceived-grievance 1.5 -0.5 ] ]
end
...
283
watch one
set visualization \"3D\"\nask max-one-of agents [perceived-grievance]\n  [ set size 2 watch-me ]
NIL
1
T
OBSERVER
