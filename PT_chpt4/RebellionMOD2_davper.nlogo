...
globals [
  k                   ; Factor for determining activation probability
  threshold           ; Threshold for rebellion activation
]
...
agents-own [
  ...
  state               ; Current state of the agent: "Quiet," "Active," or "Jailed"
...
to update-beliefs
  ; Calculate perceived grievance
  set perceived-grievance (grievance - risk-aversion * estimated-arrest-probability)
end
...
to state-machine
  ...
  if state = "Quiet" [
    ifelse (perceived-grievance > threshold) [
      set state "Active"
    ] [
      ; Remain in the quiet state
      ;set state "Quiet"
    ]
  ]
  if state = "Active" [
  ...
  if state = "Jailed" [
    ifelse jail-term = 0 [
      ; Release agent from jail and return to quiet state
      set state "Quiet"
    ] [
      ; Decrement jail term
...
to enforce
  if any? (agents-on neighborhood) with [ State = "Active" ] [
    ...
    ask suspect [
      ; Enforcement: Cops arrest active agents they encounter
      set state "Jailed"
...
; Copyright 2004 Uri Wilensky. Modified by David 2024
...
455
quiet (green)
count agents with [State = \"Quiet\" and jail-term = 0]
1
1
11
...
"" ""
PENS
"quiet" 1.0 0 -10899396 true "" "plot count agents with [State = \"Quiet\" and jail-term = 0]"
